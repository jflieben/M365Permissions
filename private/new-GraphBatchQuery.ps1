function New-GraphBatchQuery {
    ...
    .PARAMETER UseBatchApi
        Switch to use the Graph Batch API instead of making individual requests. This is more efficient for large numbers of items.
    ...
    function Invoke-WithRetry {
        ...
        while ($retryCount -le $MaxRetries) {
            try {
                if ($retryCount -gt 0) {
                    Write-LogMessage -level 3 -message "Retry attempt $retryCount for $Name"
                }
                ...
                if ($_.Exception.Message -like "*Request_ResourceNotFound*" -or 
                    $_.Exception.Message -like "*Resource*does not exist*" -or
                    $_.Exception.Message -like "*404 (Not Found)*") {
                    
                    Write-LogMessage -level 3 -message "Resource not found error for $Name - not retrying: $_"
                    # Return a special marker object to indicate resource not found
                    return @{
                        status = 404
                        error  = "Resource not found: $_"
                    }
                }
                ...
                elseif ($_.Exception.Response.StatusCode -eq 429) {
                    ...
                    try {
                        ...
                        $delay = [math]::Pow(5, $retryCount)
                    }
                    
                    Write-LogMessage -level 3 -message "$Name throttled. Waiting for $delay seconds before retrying."
                    Start-Sleep -Seconds $delay
                }
                elseif ($retryCount -gt $MaxRetries) {
                    Write-LogMessage -level 2 -message "$Name failed after $MaxRetries attempts: $_"
                    throw
                }
                else {
                    # Non-throttling error - use backoff
                    $delay = [math]::Pow(5, $retryCount)
                    Write-LogMessage -level 3 -message "$Name error. Waiting $delay seconds before retry: $_"
                    Start-Sleep -Seconds $delay
                }
            }
        }
        
        return $null # If we got here, all retries failed and we return null
    }
    ...
    for ($i = 0; $i -lt $BatchItems.Count; $i += $BatchSize) {
        ...
        if ($UseBatchApi) {
            ...
            $batchResponse = Invoke-WithRetry -Action {
                New-GraphQuery -Method POST -Uri $batchApiUrl -Body $batchBody -NoRetry
            } -Name "Batch request $($i+1)" -MaxRetries $maxRetries
            ...
            if ($failedItems.Count -gt 0) {
                ...
                foreach ($failedItem in $failedItems) {
                    ...
                    $retryResult = Invoke-WithRetry -Action {
                        New-GraphQuery -Uri $url -Method GET -NoRetry
                    } -Name "Failed item retry ($($failedItem.responseId))" -MaxRetries $maxRetries
            ...
            foreach ($j in 0..($currentBatch.Count - 1)) {
                ...
                if ($url.StartsWith("/")) {
                    $url = "$($global:octo.graphUrl)/v1.0$url"
                    Write-LogMessage -level 5 -message "Processing individual item $id with URL: $url"
                }
                
                $result = Invoke-WithRetry -Action {
                    New-GraphQuery -Uri $url -Method GET -NoRetry
                } -Name "Individual item $id" -MaxRetries $maxRetries
                
                # Handle resource not found results properly
                if ($null -ne $result -and $result -is [hashtable] -and $result.ContainsKey('status') -and $result.status -eq 404) {
                    $batchResult.responses += @{
                        id     = $id
                        status = 404
                        body   = $null
                    ...
                    Write-LogMessage -level 2 -message "Resource not found for individual item $id"
                }
                else {
                    $batchResult.responses += @{
                        id     = $id
                        status = if ($null -ne $result) { 200 } else { 500 }
                        body   = $result
                    }
                }
            }
